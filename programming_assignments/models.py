...
def assignment_history_file_upload_path(instance, filename):
    print(instance)
    if type(instance) is SimpleProgrammingAssignment or type(
            instance) is Testcase:
        course = instance.assignment.course
        assignment = instance.assignment
        course_name = course.title.replace(" ", "_")
        assignment_name = assignment.name.replace(" ", "_")
        assignment_path = str(course.id) + '-' + course_name + '/' + str(
            assignment.id) + '-' + assignment_name
...
prog_lang = (('C', 'C'), ('C++', 'C++'), ('Java', 'Java'), ('Python', 'Python'),
             ('Others', 'Others'))
...
class SimpleProgrammingAssignmentHistory(models.Model):
    assignment_history = models.ForeignKey(AssignmentHistory,
...
class AdvancedProgrammingAssignment(models.Model):
    ...
    ta_allocation = models.ForeignKey(TAAllocation,
                                      on_delete=models.CASCADE,
                                      blank=True,
                                      null=True)
...
class Testcase(models.Model):
    assignment = models.ForeignKey(Assignment, on_delete=models.CASCADE)
    assignment_section = models.ForeignKey(AssignmentSection,
                                           on_delete=models.CASCADE)
    name = models.CharField(max_length=settings.MAX_CHARFIELD_LENGTH)
    ...
    input_file = models.FileField(upload_to=assignment_history_file_upload_path,
                                  null=True,
                                  blank=True)
...
class Exam(models.Model):
    ...
    allowed_ip_range = models.CharField(
