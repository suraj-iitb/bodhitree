...
def video_upload_path(instance, filename):
    ...
    return os.path.join(
        str(course.id) + '-' + course.title + '/videos/' + filename)
...
def in_video_quiz_upload_path(instance, filename):
    course = instance.chapter.course
    return os.path.join(
        str(course.id) + '-' + course.title + '/in_video_quiz_files/' +
        filename)
...
class Video(models.Model):
    ...
    doc_file = models.FileField(upload_to=video_doc_upload_path,
                                null=True,
                                blank=True)
    invideo_quiz_file = models.FileField(upload_to=in_video_quiz_upload_path,
                                         null=True,
                                         blank=True)
